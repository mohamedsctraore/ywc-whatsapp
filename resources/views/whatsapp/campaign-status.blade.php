    ...
    <div class="row">
        ...
        <div class="col-12 mb-3">
            <div class="float-right">
                <a class="lw-btn btn btn-secondary" href="{{ route('vendor.campaign.read.list_view') }}">{{ __tr('Back
                    to Campaigns') }}</a>
                <a class="lw-btn btn btn-primary" href="{{ route('vendor.campaign.new.view') }}">{{ __tr('Create New
                    Campaign') }}</a>
            </div>
        </div>
        <!--/ button -->
                ...
                <div class="card-body ">
                    <div class="row">
                        <div class="col">
                            ...
                            <div class="">{!! __tr('__scheduledAt__ as per your account timezone which is
                                __selectedTimezone__', [
                                '__scheduledAt__' => formatDateTime($campaign->scheduled_at),
                                '__selectedTimezone__' => '<strong>'. getVendorSettings('timezone') .'</strong>'
                                ]) !!} </div>
                            <div class=" text-muted">{!! __tr('Campaign scheduled on __scheduledAt__ as per the
                                __selectedTimezone__ timezone', [
        ...
        <ul class="nav nav-tabs" id="myTab" role="tablist">
            ...
            <li class="nav-item text-right col">
                <button @click="window.reloadDT('#lwCampaignQueueLog');" class="btn btn-dark btn-sm"><i class="fa fa-sync"></i> {{  __tr('Refresh log') }}</button>
            </li>
        </ul>
        <!--/end of tabs -->
        <!-- tab Container -->
